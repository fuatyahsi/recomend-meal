...
class _RecipeDetailScreenState extends State<RecipeDetailScreen> {
  ...
  static Widget _buildCategoryBadge(
      String category, String locale, ThemeData theme) {
    const categoryMap = {
      'breakfast': {
        'emoji': '🍳',
        'tr': 'Kahvaltı',
    ...
    final cat = categoryMap[category] ??
        {'emoji': '🍽️', 'tr': category, 'en': category, 'color': 0xFFEEEEEE};
  ...
  Widget build(BuildContext context) {
    ...
    final scaledPrepTime =
        (recipe.prepTimeMinutes * (0.95 + (_multiplier - 1) * 0.18))
            .round()
            .clamp(1, 999);
    final scaledCookTime =
        (recipe.cookTimeMinutes * (0.95 + (_multiplier - 1) * 0.12))
            .round()
            .clamp(1, 999);
    final missingIngredients = recipe.ingredients
        .where(
          (ingredient) =>
              ...
              !provider.isIngredientSelected(ingredient.ingredientId),
        )
        .toList();
    final substitutionHints = missingIngredients
        .map((ingredient) {
          final ingredientData =
              provider.recipeService.getIngredientById(ingredient.ingredientId);
          final substitute = IngredientSubstitutes.getSubstitute(
            ingredient.ingredientId,
            locale,
          );
          if (substitute == null) return null;
          return MapEntry(
            ingredientData?.getName(locale) ?? ingredient.ingredientId,
            substitute,
          );
        })
                ...
                child: Column(
                  ...
                  children: AnimationConfiguration.toStaggeredList(
                    ...
                    children: [
                        ...
                        child: Row(
                          ...
                          children: [
                            _InfoChip(
                                ...
                                label: l10n.prepTime,
                                value: '$scaledPrepTime\'',
                                color: Colors.blue),
                            _divider(theme),
                            _InfoChip(
                                icon: Icons.local_fire_department_outlined,
                                label: l10n.cookTime,
                                value: '$scaledCookTime\'',
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text(
                              isTr
                                  ? 'Toplam Süre: ${scaledPrepTime + scaledCookTime} dakika'
                                  : 'Total Time: ${scaledPrepTime + scaledCookTime} minutes',
                              style: TextStyle(
                      ...
                      if (substitutionHints.isNotEmpty) ...[
                        const SizedBox(height: 14),
                        Container(
                          width: double.infinity,
                          padding: const EdgeInsets.all(16),
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                isTr
                                    ? 'Eksik malzeme yuzunden tarifi birakma. En hizli ikameler burada.'
                                    : 'Do not drop the recipe because of a missing ingredient. Fast substitutes are ready here.',
                                style: TextStyle(color: Colors.amber.shade900),
                              ),
                              const SizedBox(height: 10),
                              ...substitutionHints.take(3).map((hint) {
                                return Padding(
                      ...
                      ...recipe.ingredients.map((ing) {
                        final ingredientData = provider.recipeService
                            .getIngredientById(ing.ingredientId);
                        final hasIngredient =
                            provider.isIngredientSelected(ing.ingredientId);
                        final name =
                            ingredientData?.getName(locale) ?? ing.ingredientId;
                        final icon = ingredientData?.icon ?? '🥘';
                        final substitute = IngredientSubstitutes.getSubstitute(
                            ing.ingredientId, locale);
                                        ...
                                        AnimatedSwitcher(
                                          ...
                                          child: Text(
                                            ing.getScaledAmount(
                                                locale, _multiplier),
                                            key: ValueKey(
                                                '${ing.ingredientId}_$_multiplier'),
                            ...
                            onPressed: () {
                              final ingredientNames =
                                ...
                                final data = provider.recipeService
                                    .getIngredientById(ing.ingredientId);
                                final n =
                                    data?.getName(locale) ?? ing.ingredientId;
          ...
          Row(
            children: [
              Container(
                ...
                child:
                    Icon(Icons.groups, size: 20, color: Colors.teal.shade700),
              ),
              const SizedBox(width: 10),
              Expanded(
                child: Text(
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Icon(icon,
            size: 20,
            color: enabled ? Colors.teal.shade700 : Colors.grey.shade400),
      ),
    );
  }
}
