/// Ruh Haline Göre Yemek Öneri Sistemi
/// Tarif kategorileri ve zorluk/süre bilgilerine göre mood eşleştirmesi yapar
class MoodRecipeEngine {
  static const List<MoodOption> moods = [
    ...
    MoodOption(
      ...
      color: 0xFFFF7043,
      filter: MoodFilter(maxTime: 0, difficulties: ['medium', 'hard'], categories: ['appetizer', 'main']),
    ),
    MoodOption(
      id: 'comfort',
      emoji: '🛋️',
      ...
      color: 0xFFFFB74D,
      filter: MoodFilter(maxTime: 0, difficulties: [], categories: ['soup', 'main', 'dessert']),
    ),
    MoodOption(
      id: 'healthy',
      emoji: '🥦',
      ...
      color: 0xFF66BB6A,
      filter: MoodFilter(maxTime: 0, difficulties: ['easy', 'medium'], categories: ['salad', 'soup', 'side']),
    ),
    MoodOption(
      id: 'romantic',
      emoji: '💕',
    ...
    MoodOption(
      ...
      color: 0xFF8D6E63,
      filter: MoodFilter(maxTime: 0, difficulties: [], categories: ['soup', 'main', 'side', 'breakfast']),
    ),
    MoodOption(
      id: 'party',
      emoji: '🎉',
      ...
      color: 0xFFAB47BC,
      filter: MoodFilter(maxTime: 0, difficulties: ['medium', 'hard'], categories: ['main', 'appetizer', 'dessert']),
    ),
  ];

  static MoodOption? getMoodById(String id) {
    try {
      return moods.firstWhere((m) => m.id == id);
    } catch (_) {
      return null;
    }
  }
